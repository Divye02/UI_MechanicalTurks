...
var Imagepos = React.createClass({displayName: "Imagepos",
	...
	handleChange: function(event){
			...
			if(this.state.clicks < 2){
				this.setState({clicks: this.state.clicks + 1 , position: [this.state.position, pos_x-OFFSET_X, pos_y-OFFSET_Y]}, function(){
					console.log(this.state.position);
				});
			}
			else{
				document.getElementById("pointer_div").onclick = function() { return false; } ;
			}	
	},
	render:function(){
		return(	
