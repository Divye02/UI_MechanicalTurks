...
var Imagepos = React.createClass({displayName: "Imagepos",
	...
	handleChange: function(event){
			...
			var pos_y = event.clientY?(event.clientY):event.pageY;
			if(this.state.clicks == 0){
				this.setState({x_axis: pos_x, y_axis: pos_y, clicks: this.state.clicks + 1 , position: [pos_x-OFFSET_X, pos_y-OFFSET_Y]}, function(){
					console.log(this.state.position);
				});
			}
			else if(this.state.clicks == 1){
...
var BoundingBox = React.createClass({displayName: "BoundingBox",
	...
	componentWillReceiveProps: function(){
		this.setState({x_axis: this.props.x_axis, y_axis: this.props.y_axis, width: this.props.width, height: this.props.height});
	},
	componentDidMount: function(){
		var c = document.getElementById("myCanvas");
		var ctx = c.getContext("2d");
		ctx.rect(this.state.x_axis, this.state.y_axis, this.state.width, this.state.height);
		ctx.stroke();
	},
	render : function(){

		return (
			React.createElement("canvas", {id: "myCanvas", width: this.state.width, height: this.state.height})
			);
	}
});
